use std::sync::Arc;

use egui_kittest::HarnessBuilder;
use egui_kittest::kittest::Queryable;
use nektar_impact_lx_plus_editor::ImpactLxPlusEditor;
use nektar_impact_lx_plus_editor::config::AppConfig;
use nektar_impact_lx_plus_editor::state::EditorTab;
use tokio::sync::mpsc::unbounded_channel;

#[test]
fn every_page_renders() {
    let probes = [
        (EditorTab::Presets, "Fader Buttons"),
        (EditorTab::Pads, "Pad 8"),
        (EditorTab::Controller, "Transport Buttons"),
        (EditorTab::Settings, "Unmapped Settings"),
    ];

    for (tab, expected_label) in probes {
        let (app_tx, _app_rx) = unbounded_channel();
        let (_ui_tx, ui_rx) = unbounded_channel();
        let config = Arc::new(AppConfig::default());
        let app = ImpactLxPlusEditor::new(app_tx, ui_rx, config);

        let mut harness = HarnessBuilder::default()
            .with_size(eframe::egui::vec2(1200.0, 820.0))
            .build_ui_state(
                move |ui, app: &mut ImpactLxPlusEditor| {
                    app.set_tab_for_test(tab);
                    app.render(ui);
                },
                app,
            );
        harness.run();
        let _ = harness.get_by_label(expected_label);
    }
}
