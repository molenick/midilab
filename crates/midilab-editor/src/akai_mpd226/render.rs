...
use crate::akai_mpd226::spacing::spacing;
use crate::akai_mpd226::state::UiState;
use crate::akai_mpd226::state::UserSelection;
                ...
                ui.horizontal(|ui| {
                    ...
                    if let Some(status) = &ui_state.user_msg {
                        let color = match status.kind {
                            crate::akai_mpd226::message::UserMsgKind::Status => Color32::GREEN,
                            crate::akai_mpd226::message::UserMsgKind::Error => Color32::RED,
                        };
...
fn settings_modal(ctx: &Context, ui_state: &mut UiState, outbox: &mut Vec<UiEffect>) {
    let modal_response = Modal::new(egui::Id::new("settings_modal")).show(ctx, |ui| {
            ...
            .clicked()
        {
            ...
            let config = crate::akai_mpd226::config::AppConfig {
                persistence_path: ui_state.configured_directory.clone(),
                user: ui_state.user_settings.clone(),
            };
            let config_path = crate::akai_mpd226::config::AppConfig::config_path()
